...
/** ADMIN — post an announcement at any scope. Mirrors {@code POST /announcements}. */
@Component
public class PostAnnouncementTool extends AbstractActionTool {
  ...
  protected PrepResult prepare(JsonNode args, ToolContext ctx) {
    ...
    impact.put("target", target);
    impact.put("body", body);

    String en = "I'll post this to " + target + " (parents notified): \"" + body + "\". Confirm?";
    String ar = "سأرسل هذا إلى " + target + " (سيُخطَر أولياء الأمور): «" + body + "». أؤكّد؟";
